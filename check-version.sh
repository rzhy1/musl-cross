...
if version_gt "zlib" "$zlib_tag1" "$zlib_tag"; then
  echo "- zlib有最新版：$zlib_tag1  $zlib_latest_url" | tee -a "${VERSION}"
  update_found=true
else
  echo "- zlib ${zlib_tag} 已是最新版 https://github.com/madler/zlib/releases/download/v${zlib_tag}/zlib-${zlib_tag}.tar.xz" | tee -a "${VERSION}"
fi
...
if version_gt "zstd" "$zstd_tag1" "$zstd_tag"; then
  echo "- zstd有最新版：$zstd_tag1  $zstd_latest_url" | tee -a "${VERSION}"
  update_found=true
else
  echo "- zstd ${zstd_tag} 已是最新版 https://github.com/facebook/zstd/releases/download/v${zstd_tag}/zstd-${zstd_tag}.tar.gz" | tee -a "${VERSION}"
fi
...
if version_gt "gmp" "$gmp_tag1" "$gmp_tag"; then
  echo "- gmp有最新版：$gmp_tag1  https://ftp.gnu.org/gnu/gmp/gmp-${gmp_tag1}.tar.xz" | tee -a "${VERSION}"
  update_found=true
else
  echo "- gmp ${gmp_tag} 已是最新版 https://ftp.gnu.org/gnu/gmp/gmp-${gmp_tag}.tar.xz" | tee -a "${VERSION}"
fi
...
if version_gt "isl" "$isl_tag1" "$isl_tag"; then
  echo "- isl有最新版：$isl_tag1  https://libisl.sourceforge.io/isl-${isl_tag1}.tar.xz" | tee -a "${VERSION}"
  update_found=true
else
  echo "- isl ${isl_tag} 已是最新版 https://libisl.sourceforge.io/isl-${isl_tag}.tar.xz" | tee -a "${VERSION}"
fi
...
if version_gt "mpfr" "$mpfr_tag1" "$mpfr_tag"; then
  echo "- mpfr有最新版：$mpfr_tag1  $mpfr_tag1  https://ftp.gnu.org/gnu/mpfr/mpfr-${mpfr_tag1}.tar.xz" | tee -a "${VERSION}"
  update_found=true
else
  echo "- mpfr ${mpfr_tag} 已是最新版 https://www.mpfr.org/mpfr-${mpfr_tag}/mpfr-${mpfr_tag}.tar.xz" | tee -a "${VERSION}"
fi
...
if version_gt "mpc" "$mpc_tag1" "$mpc_tag"; then
  echo "- mpc有最新版：$mpc_tag1  https://ftp.gnu.org/gnu/mpc/mpc-${mpc_tag1}.tar.gz" | tee -a "${VERSION}"
  update_found=true
else
  echo "- mpc ${mpc_tag} 已是最新版 https://www.multiprecision.org/downloads/mpc-${mpc_tag}.tar.gz" | tee -a "${VERSION}"
fi
...
if version_gt "binutils" "$binutils_tag1" "$binutils_tag"; then
  echo "- binutils有最新版：$binutils_tag1  https://ftp.gnu.org/gnu/binutils/binutils-${binutils_tag1}.tar.xz" | tee -a "${VERSION}"
  update_found=true
else
  echo "- binutils ${binutils_tag} 已是最新版 https://ftp.gnu.org/gnu/binutils/binutils-${binutils_tag}.tar.xz" | tee -a "${VERSION}"
fi
...
if version_gt "gcc" "$gcc_tag1" "$gcc_tag"; then
  ...
  echo "- gcc ${gcc_tag} 已是最新版 https://ftp.gnu.org/gnu/gcc/gcc-${gcc_tag1}/gcc-${gcc_tag1}.tar.xz" | tee -a "${VERSION}"
fi
